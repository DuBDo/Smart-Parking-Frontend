...
import { setParkingLot, setUser } from "../../redux/userSlice";
  ...
  const handleListing = async () => {
    try {
      ...
      const response = await axios.post(
        `${import.meta.env.VITE_BACKEND_URL}/api/V1/parking-lot`,
        formData,
        {
          headers: {
      ...
      dispatch(setParkingLot(response.data.parkingListed));
      navigate("/dashboard");
    } catch (error) {
  ...
  return (
    <div className="relative min-h-screen w-1/2 bg-white flex flex-col">
      ...
      <div className="flex-1 flex flex-col mt-14 px-20">
        ...
        <p className="text-md">
          <span className="font-medium">Note: </span>You will need to be
          verified to rent out your space it won't take longer than 24hrs.
        </p>
      ...
      <div className="w-full border-t border-t-gray-300 mt-8 py-6 flex justify-between">
        <button
          className="ml-5 cursor-pointer text-green-600 font-medium"
          onClick={() => setStep(6)}
        >
          Go back
        </button>
