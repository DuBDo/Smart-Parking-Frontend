...
const NavBar = () => {
    ...
    const userDropdownLinks = [
        ...
        { path: "/dashboard/bookings-received", label: "Billings" },
    ...
    return (
        <nav className="shadow-md sticky top-0 z-50 bg-[#f6f8fb]">
            <div className="relative max-w-7xl mx-auto px-4 sm:px-8 flex justify-between items-center h-20">
                {/* Logo */}
                <Link to="/" className="text-3xl font-extrabold text-[#1fa637]">
                    ...
                    {[...navLinks, ...authLinks].map((link) => (
                        link.isDropdown ? (
                            ...
                            >
                                ...
                                {isUserMenuOpen && (
                                    <div 
                                        className="absolute flex flex-col h-auto top-full right-0 mt-3 w-52 bg-white max-w-md border border-gray-200 rounded-md shadow-lg z-20 overflow-hidden"
                                    >
                                        {userDropdownLinks.map(dropdownLink => (
                                            <Link 
                                                ...
                                                onClick={(e) => {
                                                    // Optional: Prevent link navigation if you handle routing elsewhere
                            ...
                            <NavLink
                                ...
                                to={link.path}
                                className={({ isActive }) =>
                                    `font-medium transition-colors duration-200 **no-underline** ${
                                        isActive
