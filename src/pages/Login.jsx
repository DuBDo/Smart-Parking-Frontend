...
function Login() {
  ...
  const handleSignIn = async (e) => {
    ...
    try {
      ...
      if (data.parkingLot) {
        dispatch(setParkingLot({ parkingLot: data.parkingLot }));
      }
      navigate("/home");
    } catch (error) {
  ...
  return (
    <div className="min-h-screen flex justify-center items-center bg-[#f8f9fb]">
      <div className="pl-7 pr-8 py-12 min-w-3xl shadow rounded bg-white shadow-gray-300">
        ...
        <div className="pl-2">
          <h1 className="text-xl font-[400] py-9">
          ...
          <form action="">
            ...
            <div className="mt-12 mb-3 flex items-center justify-between">
              <button
                className="text-[#1fa637] text-lg cursor-pointer"
                onClick={() => navigate("/")}
              >
                Cancel
              </button>
              <button
                className="px-14 py-3 text-white rounded-lg font-medium text-lg cursor-pointer bg-[#1fa637] hover:bg-green-700"
